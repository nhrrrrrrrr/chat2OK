...
const useSendGroupMessage = () => {
  ...
  const { Groupmessages, selectedgroupConversation, setGroupMessages } =
    useConversation();
  ...
  const sendMessage = async (message) => {
    ...
    try {
      ...
      const res = await fetch(
        `/api/messages/sendgroup/${selectedgroupConversation._id}`,
        {
          method: "POST",
          headers: {
      ...
      console.log(data);
      setGroupMessages([...Groupmessages, data]); //这里是为了更新 sender发送者方的信息messages
    } catch (error) {
